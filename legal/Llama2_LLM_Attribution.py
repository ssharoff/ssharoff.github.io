...
eval_prompt = open(sys.argv[1]).read() if len(sys.argv)>1 else "Dave lives in Palm Coast, FL and is a lawyer. His personal interests include"
target = open(sys.argv[2]).read() if len(sys.argv)>2 else "playing guitar, hiking, and spending time with his family."
fname=re.sub('[^a-z0-9]+','',sys.argv[1])

# an extra parameter to run the expensive ShapleyValue
dosv = True if len(sys.argv)>3 else False
...
loadtime=time.time()
print(f"Loaded in {loadtime-starttime} sec")
# sv = ShapleyValues(model) 
if dosv:
    sv = ShapleyValueSampling(model) 
    sv_llm_attr = LLMAttribution(sv, tokenizer)

    attr_res = sv_llm_attr.attribute(inp, target=target, num_trials=5)
    print(dir(attr_res))

    attr_res.plot_token_attr(show=False)[0].savefig(f"{sys.argv[2]+fname}-SHAP-Sample.pdf", format="pdf", bbox_inches="tight")
    print(attr_res.plot_token_attr(show=False)[1])
svtime=time.time()
print(f"Shapley values computed in {svtime-loadtime} sec")
...
print(dir(attr_res))
igtime=time.time()
print(f"IG values computed in {igtime-svtime} sec")

attr_res.plot_seq_attr(show=False)[0].savefig(f"{sys.argv[2]+fname}-attr-IG.pdf", format="pdf", bbox_inches="tight")
...
print(attr_res.seq_attr_dict)
print(attr_res.seq_attr_dict)
attr_res.plot_token_attr(show=True)
print(dir(attr_res.plot_token_attr()))
attr_res.plot_token_attr(show=False)[0].savefig(f"{sys.argv[2]+fname}-IG.pdf", format="pdf", bbox_inches="tight")
