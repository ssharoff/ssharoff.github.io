...
# upd. by karev
# $goodchars='[а-яА-Я]'; #only Russian words to lemmatise automatically by defaulta
$goodchars='[а-яА-ЯёЁ]'; #only Russian words to lemmatise automatically by defaulta
...
my ($corpusfh, $corpusfile) = tempfile(DIR=>$tmpdirname); # for storing the corpus
binmode($corpusfh,":utf8");

my ($lemmafh, $lemmafile) = tempfile(DIR=>$tmpdirname); #for storing unknown lemmas
binmode($lemmafh,":utf8");

#print STDERR "$corpusfile,$lemmafile\n";

unless (defined $patterns) {
    close($corpusfh);
    print STDERR "Warning: no patterns specifed: @ARGV\n";
    $corpusfh=*STDOUT;
};
